module.exports = app => {
	...
	login = (req, res) => {
		...
		userDao.findUserByCredentials(credentials).then(user => {
			if (user !== null) {
	...
	getProfileOfUser = (req, res) => {
		...
		userDao.findUserByUsername(username).then(users => res.json(users[0]));
	};
	...
	updateProfile = (req, res) => {
		...
		delete newUser._id;
		userDao.updateUser(userId, newUser).then(status => {
	...
	createUser = (req, res) => {
		var user = req.body;
		if (user._id == null) user._id = new Date().getTime();
		user.dateCreated = new Date();
		userDao.findUserByUsername(user.username).then(users => {
			if (users.length === 0) {
